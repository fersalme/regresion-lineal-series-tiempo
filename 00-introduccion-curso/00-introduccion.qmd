...
## Evaluación
- **Ejercicios y talleres prácticos en laboratorios: 40%**
    - Por semana hay actividad grupales o individuales en el laboratorio.
- **Quiz: 20%**
    - Quiz escritos durante el curso.
- **Parcial del corte: 40%**
    - Puede ser un exámen o desarrollo de un proyecto en el laboratorio.
...
- Python : Crea un entorno con :
    - pip : `python -m venv time-series` y actívalo con `source time-series/bin/activate` (Linux/Mac) o `.\time-series\Scripts\activate` (Windows). Luego, para instalar los paquetes usa  `pip install -r py-requirements.txt` para instalar paquetes desde `py-requirements.txt`.
    - conda : `conda create --name time-series python=3.13` y actívalo con `conda activate time-series`. Luego, para instalar los paquetes usa  `pip install -r py-requirements.txt` para instalar paquetes desde `py-requirements.txt`.
...
- Horario de atención a estudiantes: Lunes a Viernes 2 PM - 5PM por Microsoft-Teams.

...
author: "Fernando Salcedo Mejía, Economista, MSc"
institute: "Escuela de Transformación Digital - ETD | Programa de Ciencia de Datos"
date: "Febrero 2026"
format:
  revealjs:
...
- Estimar e interpretar los coeficientes de un modelo de regresión lineal aplicado a series de tiempo mediante Mínimos Cuadrados Ordinarios (MCO).
- Verificar los supuestos del modelo de regresión lineal (linealidad, homocedasticidad, no autocorrelación, exogeneidad) y diagnosticar su cumplimiento a partir de los residuos.
- Construir predictores temporales (tendencia, variables dummy, series de Fourier) para capturar patrones estacionales.
- Generar pronósticos con sus respectivos intervalos de confianza y evaluar su precisión con métricas como AIC, BIC, RMSE, MAE, MAPE y MASE.
...
\mathbf{\varepsilon}'\mathbf{\varepsilon} = \mathbf{y}'\mathbf{y}  - 2\mathbf{y}'\mathbf{X}\mathbf{\beta} + \mathbf{\beta}'\mathbf{X}'\mathbf{X}\mathbf{\beta}
$$

- Minimizando la expresión tenemos :
$$
\frac{\partial \mathbf{\varepsilon}'\mathbf{\varepsilon}}{\partial \beta} = -2\mathbf{X}'\mathbf{y} + 2\mathbf{X}'\mathbf{X}\mathbf{\beta}  = 0 \\
$$

 - Resolviendo para $\beta$
$$
2\mathbf{X}'\mathbf{X}\mathbf{\beta} = 2\mathbf{X}'\mathbf{y} \\
...
- **Independencia** : $Y_i, .. Y_n$ son independientes
...
- **Colinealidad perfecta** : no existe correlación perfecta entre las regresoras incluidas en el modelo de regresión $X_1 = 2X_2$
...
$$
Var(\hat{\beta_0}) = \sigma^2\frac{\sum X^2_i}{n\sum(X_i - \overline{X})^2}, Var(\hat{\beta_1}) = \frac{\sigma^2}{\sum(X_i - \overline{X})^2}
$$
...
## Ejercicio 1. Graficar la convergencia de los parámetros estimados por MCO a sus verdaderos valores.
...
$$
t_{1 - \alpha/2; (n - k)} = \frac{\hat{\beta_j} - \beta_j}{\sqrt{Var(\hat{\beta_j})}}
$$
- **Regla de decisión :** Si $P(t_{1 - \alpha/2; (n - k)}) < 0.05$ consideramos que hay evidencia suficiente contra $H_0$, por lo que se acepta $H_a$.
$$
...
::: {.callout-note}
Este bloque se muestra como referencia (`eval: false`) y no se ejecuta al renderizar, ya que el motor del documento (`engine: jupyter`) corre la solución en Python. Puedes ejecutarlo de forma independiente en una sesión de R.
:::
...
- La relación de las variables regresoras deben ser lineales con la variables explicada. Gráficamente, no debemos ver un patrón funcional entre los errores y los valores predichos del modelo.
...
- Los errores del modelo deben distribuirse de forma constante. Gráficamente, no debemos ver un patrón entre los errores y los valores predichos del modelo.
...
- En cambio, cuando los datos son estacionales, las autocorrelaciones serán mayores para los rezagos estacionales (semanales, trimestrales, anuales, etc) que para otros rezagos.
...
- H0 : Los residuos no están correlacionados hasta el $\ell$ rezagos (**no son** distintos de un proceso ruido blanco)
- Ha : Al menos uno de los residuos está correlacionado hasta el $\ell$ rezago
...
- **Regla de decisión :** Si $P(Q^*) > 0.05$ la serie sigue un proceso ruido blanco.
...
### Tendencia
- La tendencia $T$ se puede modelar incluyéndola como variable explicativa en el modelo
$$
...
- Crear variables estacionales en los datos, tal que capturen patrones específicos de esos momentos. Puedes capturar picos específicos (como Navidad o vacaciones de verano) que una tendencia lineal simple ignoraría.
$$
...
- Un problema que podemos enfrentar es cuando la serie tiene una estacional alta como las series semanales, ya que por año son 52 semanas, lo cual termina en 51 variables dummy estacionales. Recordemos que para cada variable el estimador _MCO_ requiere estimar un $\hat{\beta}$ lo cual es más difícil en la medida que incluimos más variables con los mismos datos.

### Series de Fourier 

- Es una alternativa a las dummy estacionales, en especial en series con estacionalidad alta. Jean-Baptiste Fourier un matemático Francés describió que las funciones seno y coseno pueden usarse para aproximarse a cualquier serie periódica. Aquí vemos las primeras pares de la serie de seno y cosenos para una estacional de una semana :
...
- El pronóstico del modelo estimado no es más que usar los $\hat{\beta}$ encontrados por _MCO_ sobre una nueva observación de $X$
$$
\hat{y_t} = \hat\beta_{0} + \hat\beta_{1} x_{1,t} + \hat\beta_{2} x_{2,t} + \cdots + \hat\beta_{k} x_{k,t}
$$

- Además un intervalo de confianza al 95% para el pronóstico (asumiendo normalidad en los errores del modelo) se define como :
$$
\hat{y} \pm 1.96 \hat{\sigma}_e\sqrt{1+\frac{1}{T}+\frac{(x-\bar{x})^2}{(T-1)s_x^2}}
$$

::: {.callout-note title="Pronóstico ex-ante y ex-post"}
- **Pronóstico ex-ante :** Son aquellos que se realizan únicamente con los datos que están disponibles previamente de los predictores del modelo.
- **Pronóstico ex-post :** Son aquellos que se realizan utilizando información posterior o nueva sobre los predictores del modelo.
:::

- Otra forma de hacer pronósticos es con la construcción de escenarios, donde suponemos que las variables de nuestro modelo toman un rango de valores esperados en el futuro. Esto es útil para tomadores de decisión si quieren ver valores esperados de la serie bajo supuestos y hacer comparaciones.

## Ejercicio 3:

- Hacer una regresión del consumo con el ingreso en el modelo macroeconómico de Estados Unidos. El ministro de economía del país está interesado en saber cuál será el consumo esperado en el siguiente trimestre en tres escenarios : si el ingreso se mantiene en sus valores promedios históricos, se reduce 0.2 puntos porcentuales o aumenta 0.8 puntos porcentuales respecto al promedio histórico.
...
- Cuando tenemos varios modelos con diferentes covariables **pero con la misma variable dependiente** podemos usar estadísticos de selección de modelos como :
...
- Para hacer pronósticos, nos interesa verificar la precisión del modelo usando los errores de estimación $\varepsilon_t = y_t - \hat{y}_t$. Para ello, verificamos qué tanto se equica mi modelo con los datos, donde vamos a seleccionar el modelo con el menor error posible. **En la práctica necesitamos dividir la serie en un segmento para estimar el modelo y otro para verificar el rendimiento.**
...
$$
- Raíz del error cuadrático escalado (RMSSE) : $\text{RMSSE} = \sqrt{\text{mean}(q_{j}^2)}$

- Una técnica más avanzada es la validación cruzada (CV), donde el objetivo es mover el punto de cohorte del modelo e ir verificando el rendimiento en el conjunto de datos posterior. Luego se promedia un estadístico de precisión del pronóstico (usualmente MAE):
...
- La regresión lineal, estimada por MCO, permite modelar y pronosticar una serie de tiempo a partir de su relación con otras variables.
- El estimador MCO es insesgado, de varianza mínima y consistente **si y solo si** se cumplen sus supuestos (linealidad, homocedasticidad, no autocorrelación, exogeneidad, no colinealidad perfecta).
- En datos temporales, la autocorrelación de los residuos es la violación más común: se diagnostica con ACF, Durbin-Watson y Ljung-Box, y se corrige agregando rezagos, diferenciando o incluyendo variables estacionales.
- Predictores como la tendencia, las variables dummy estacionales y las series de Fourier permiten capturar el comportamiento temporal dentro de un modelo lineal.
- La selección del mejor modelo combina criterios de ajuste penalizado ($\bar R^2$, AIC, BIC) con medidas de error de pronóstico fuera de muestra (RMSE, MAE, MAPE, MASE) y validación cruzada.

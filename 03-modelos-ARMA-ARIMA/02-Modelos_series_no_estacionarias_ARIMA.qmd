...
$$
\Delta \ln(y_t) = \ln(y_t) - \ln(y_{t-1}) \approx \frac{y_t - y_{t-1}}{y_{t-1}}
$$

- La diferencia del logaritmo aproxima la tasa de crecimiento porcentual de $y_t$ entre $t-1$ y $t$.

## Ejemplo: $\Delta \ln(y_t)$ vs. tasa de crecimiento porcentual
...
np.random.seed(42)
n = 60
tasas_crecimiento = np.random.normal(0.02, 0.05, n)
y = pd.Series(
    100 * np.cumprod(1 + tasas_crecimiento),
    index=pd.date_range("2015-01-01", periods=n, freq="QE"),
)

log_diff = np.log(y).diff()
...
axes[0].plot(
    log_diff.index,
    log_diff,
    label=r"$\Delta \ln(y_t)$",
    color="darkorange",
    linestyle="--",
    marker="x",
)
axes[0].axhline(0, color="black", linewidth=0.8)
axes[0].set_title("Comparación en el tiempo")
axes[0].set_xlabel("Tiempo")
axes[0].set_ylabel("Tasa de cambio")
axes[0].legend()

# Dispersión: una medida contra la otra
axes[1].scatter(pct_change, log_diff, color="teal", alpha=0.7)
lims = [min(pct_change.min(), log_diff.min()), max(pct_change.max(), log_diff.max())]
axes[1].plot(lims, lims, color="red", linestyle="--", label="y = x")
axes[1].set_title(r"$\Delta \ln(y_t)$ vs. $(y_t - y_{t-1})/y_{t-1}$")
axes[1].set_xlabel(r"$(y_t - y_{t-1})/y_{t-1}$")
axes[1].set_ylabel(r"$\Delta \ln(y_t)$")
axes[1].legend()
...
- Para variaciones pequeñas entre periodos, ambas medidas son prácticamente idénticas (los puntos caen sobre la línea $y=x$).
- La diferencia entre ambas crece con la magnitud del cambio, ya que $\Delta \ln(y_t)$ es una aproximación de primer orden de la tasa de crecimiento porcentual.
...
$$
y_{T+1} =  (1 + \phi) y_{T + 1 - 1 } - \phi  y_{T + 1 - 2} + \varepsilon_{T + 1} + \theta \varepsilon_{T + 1 - 1} \\
y_{T+1} =  (1 + \phi ) y_{T} - \phi  y_{T - 1} + \varepsilon_{T + 1} + \theta \varepsilon_{T}
$$
...
$$
y_t =  (1 + \phi)y_{t-1} - \phi  y_{t-2} + \varepsilon_t + \theta \varepsilon_{t - 1} \\
y_{T+2} =  (1 + \phi) y_{T + 2 - 1 } - \phi  y_{T + 2 - 2} + \varepsilon_{T + 2} + \theta \varepsilon_{T + 2 - 1} \\
...
**Criterios de información (dentro de muestra)**: penalizan modelos con más parámetros $k$; se usan para comparar modelos con la **misma** variable dependiente. Menor valor = mejor modelo.

- **AIC**: $-2\log(L) + 2k$
- **BIC**: $-2\log(L) + k\log(n)$

**Métricas de error (fuera de muestra)**: requieren separar los datos en entrenamiento/prueba para evaluar la capacidad real de pronóstico del modelo.
...
- **MASE** (Mean Absolute Scaled Error): $\frac{1}{n}\sum\frac{|y_i - \hat{y}_i|}{\frac{1}{n-1}\sum|y_t - y_{t-1}|}$, evita los problemas de MAPE cuando $y_i \approx 0$.

## Ejemplo: métricas del modelo ARIMA(1,1,1)

```{python}
from sklearn.metrics import mean_absolute_error, mean_squared_error

y_hat = modelo_arima.fittedvalues
mae = mean_absolute_error(y, y_hat)
rmse = np.sqrt(mean_squared_error(y, y_hat))
mape = np.mean(np.abs((y - y_hat) / y)) * 100
escala_mase = np.mean(np.abs(np.diff(y)))  # error del pronóstico ingenuo
mase = mae / escala_mase

print(f"AIC: {modelo_arima.aic:.2f}")
print(f"BIC: {modelo_arima.bic:.2f}")
print(f"MAE: {mae:.3f}")

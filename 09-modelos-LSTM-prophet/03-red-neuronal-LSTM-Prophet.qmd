...
- Prophet incluye una funcionalidad para realizar validación cruzada de series temporales con el fin de medir el error de pronóstico utilizando datos históricos.

- Especificamos el horizonte de pronóstico (`horizon`) y, opcionalmente, el tamaño del período de entrenamiento inicial (`initial`) y el intervalo entre las fechas de corte (`period).

- En una serie temporal de 8 años:
  - Horizonte (365 días): Es cuánto tiempo hacia el futuro intenta predecir el modelo en cada intento.
  - Entrenamiento inicial (730 días): El modelo necesita una base de datos para aprender.
  - Periodo de salto (180 días): Después de cada predicción, el _punto de corte_ se desplaza hacia adelante 180 días.
  - Este proceso se repite hasta cubrir los 8 años de la serie, resultando en 11 pronósticos totales.
...
- La validación cruzada puede ajudar a ajustar hiperparámetros especialmente los _prior_ con que el modelo estima los cambios de la tendencia y estacionalidad del modelo. 
- `changepoint_prior_scale` : Impacta el ajuste del modelo en la tendencia. El ajusta va de [0.001, 0.5] donde un valor pequeño el modelo pierde ajuste y es grande tiende al sobreajuste.
- `seasonality_prior_scale` : Ajusta la flexibiliad de la estacionalidad del modelo. El ajuste va de [0.01, 10] donde de manera similar, un valor grande permite que la estacionalidad se ajuste a grandes fluctuaciones, mientras que un valor pequeño reduce la magnitud de la estacionalidad.

## Un comentario acerca de Prophet

- A pesar de la sostificación de Prophet, comparado con modelos SARIMA, ETS, y Modelos dinámicos la ganancia en precesición no es mucha. Así lo comenta Hyndman y Athanasopoulos:

> Prophet has the advantage of **being much faster** to estimate than the dynamic harmonic regression (DHR) models we have considered previously, and it is **completely automated**. However, it **rarely gives better forecast** accuracy than the alternative approaches, as these two examples have illustrated. [Forecasting: principles and practice, 3rd edition. Prophet model](https://otexts.com/fpp3/prophet.html)
...
| **Entrenamiento** | Puede ser prolongado (GPU recomendada) | Segundos a minutos |
